...
const ExcavationProcessCarousel = ({ steps }) => {
    ...
    <div className="bg-white rounded-lg shadow-lg overflow-hidden">
      ...
      <div className="bg-amber-600 text-white p-4">
        <div className="flex justify-between items-center">
          <div>
            <h3 className="text-xl font-bold">Excavation Process</h3>
            <p className="text-amber-200 text-sm">Step {currentStep + 1} of {steps.length}</p>
          </div>
          <div className="flex items-center space-x-4">
            <button
              onClick={() => setIsAutoPlaying(!isAutoPlaying)}
              className="bg-amber-700 hover:bg-amber-800 p-2 rounded-full transition-colors"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 items-start">
              ...
              <div className="relative w-full max-w-xs aspect-[4/3] bg-gray-200 rounded-lg overflow-hidden">
                {currentStepData.image ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-amber-100">
                    <span className="text-amber-600 font-bold text-2xl md:text-4xl">{currentStepData.step}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="lg:col-span-2">
              ...
              <div className="space-y-1 md:space-y-2">
                {currentStepData.activities.slice(0, 4).map((activity, index) => (
                  <div key={index} className="flex items-start">
                    <CheckCircle className="h-3 w-3 md:h-4 md:w-4 text-amber-500 mr-2 md:mr-3 mt-0.5 flex-shrink-0" />
                    <span className="text-gray-600 text-xs md:text-sm">{activity}</span>
                  </div>
                ))}
                {currentStepData.activities.length > 4 && (
                  <p className="text-gray-500 text-xs italic ml-5 md:ml-7">
                    + {currentStepData.activities.length - 4} more activities...
                  </p>
                )}
              </div>

              {/* Timeframe Badge */}
              <div className="mt-3 md:mt-4 inline-flex items-center bg-amber-50 border border-amber-200 rounded-full px-3 md:px-4 py-1 md:py-2">
                <Clock className="h-3 w-3 md:h-4 md:w-4 text-amber-600 mr-1 md:mr-2" />
                <span className="text-amber-700 font-medium text-xs md:text-sm">{currentStepData.timeframe}</span>
              </div>
            </div>
          </div>
        </div>
...
export default ExcavationProcessCarousel;